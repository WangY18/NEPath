...
namespace nepath
{
    ...
    path Connector::FermatSpiral_SingleMinimum(pathnode *root, double dis, double in /*=-1.0*/)
    {
        ...
        while (root)
        {
            ...
            { // This circle is large enough.
                if (std::abs(in - out) < 1e-6)
                {
                    ...
                    { // Consider the last upper circle.
                        ...
                        if (std::min(dis0in + dis1out, dis1in + dis0out) < std::min(dis0in + dis2out, dis2in + dis0out))
                ...
                else
                {
                    ...
                    if (std::min(dis_in0in + dis_out1out, dis_in0out + dis_out1in) <=
                        std::min(dis_in1in + dis_out0out, dis_in1out + dis_out0in))
                    {
            ...
            { // The last circle
                int idm = 0;
                double dism = std::min(dis(root->data.x[0], root->data.y[0], xin[xin.size() - 1], yin[yin.size() - 1]),
                                       dis(root->data.x[0], root->data.y[0], xout[xout.size() - 1], yout[yout.size() - 1]));
                for (int i = 1; i < root->data.length; ++i)
                {
                    double disnow = std::min(dis(root->data.x[i], root->data.y[i], xin[xin.size() - 1], yin[yin.size() - 1]),
