...
int main()
{
    ...
    cout << "CP-DFS: There are " << CP_paths.size() << " continuous toolpaths in total." << endl;
    ...
    FileAgent::mkdir((fs::path(__FILE__).parent_path().parent_path() / "data_examples" / "demo_CP_DFS").string().c_str(), true);
    FileAgent::write_csv(CP_paths, (fs::path(__FILE__).parent_path().parent_path() / "data_examples" / "demo_CP_DFS" / "").string().c_str(), ".csv");
