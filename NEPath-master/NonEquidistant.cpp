...
IQOP_NLP::IQOP_NLP(const path& p, const NonEquidistantOptions& opts)
    ...
    for (int i = 0; i < p_.length; ++i) {
        c_A_[i] = 0.5 * ((p_.y[(i + 1) % p_.length] - p_.y[(i + p_.length - 1) % p_.length]) * nx_[i]
                      - (p_.x[(i + 1) % p_.length] - p_.x[(i + p_.length - 1) % p_.length]) * ny_[i]);
...
bool IQOP_NLP::get_nlp_info(Index& n, Index& m, Index& nnz_jac_g,
                            Index& nnz_h_lag, IndexStyleEnum& index_style) {
    // Number of variables (deltas)
    n = n_vars_;
...
bool IQOP_NLP::get_bounds_info(Index n, Number* x_l, Number* x_u,
...
bool IQOP_NLP::get_starting_point(Index n, bool init_x, Number* x,
...
bool IQOP_NLP::eval_f(Index n, const Number* x, bool new_x, Number& obj_value) {
...
bool IQOP_NLP::eval_grad_f(Index n, const Number* x, bool new_x, Number* grad_f) {
...
bool IQOP_NLP::eval_g(Index n, const Number* x, bool new_x,
                     Index m, Number* g) {
    ...
    for (int i = 0; i < n_vars_; ++i) {
        ...
        double lhs = -dl_i * dl_i * x[i_prev]
                   + (dl_i * dl_i - dl_prev * dl_prev) * x[i]
                   + dl_prev * dl_prev * x[i_next];
...
bool IQOP_NLP::eval_jac_g(Index n, const Number* x, bool new_x,
...
bool IQOP_NLP::eval_h(Index n, const Number* x, bool new_x,
...
void IQOP_NLP::finalize_solution(SolverReturn status, Index n,
...
double* NonEquidistant::Optimize_QSL(path p, const NonEquidistantOptions& opts) {
    ...
    // Create NLP problem
    SmartPtr<IQOP_NLP> nlp = new IQOP_NLP(p, opts);
