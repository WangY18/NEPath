...
namespace nepath
{
    ...
    paths ContourParallel::OffsetClipper(const double *x, const double *y, double dis, int length, bool wash /*=true*/, double washdis /*=0.5*/, int num_least /*=50*/)
    {
        ...
        {
            contour << ClipperLib::IntPoint(double2cInt(x[i], scale, delta_x), double2cInt(y[i], scale, delta_y));
        }
        contour << contour[0];
        ClipperLib::Paths solution;
        ClipperLib::ClipperOffset co;
        co.AddPath(contour, ClipperLib::jtRound, ClipperLib::etClosedPolygon);
    ...
    paths ContourParallel::Paths2paths(const ClipperLib::Paths &Ps, double scale, double delta_x /*=0.0*/, double delta_y /*=0.0*/)
    {
        paths ps;
    ...
    path ContourParallel::Path2path(const ClipperLib::Path &P, double scale, double delta_x /*=0.0*/, double delta_y /*=0.0*/)
    {
        path p;
        p.length = P.size();
    ...
    paths ContourParallel::cut_holes(const path &contour, const paths &holes, bool wash /*=true*/, double washdis /*=0.5*/, int num_least /*=50*/)
    {
        ...
        {
            Contour << ClipperLib::IntPoint(double2cInt(contour.x[i], scale, delta_x), double2cInt(contour.y[i], scale, delta_y));
        }
        Contour << Contour[0];
        ...
        for (int i_hole = 0; i_hole < holes.size(); ++i_hole)
        {
            ...
            {
                Holes[Holes.size() - 1] << ClipperLib::IntPoint(double2cInt(holes[i_hole].x[i], scale, delta_x), double2cInt(holes[i_hole].y[i], scale, delta_y));
            }
            Holes[Holes.size() - 1] << Holes[Holes.size() - 1][0];
            if (path_subset(Holes[Holes.size() - 1], Contour))
        ...
        else
        {
            ...
            cl.AddPath(Contour, ClipperLib::ptSubject, true);
            cl.AddPaths(Holes, ClipperLib::ptClip, true);
            cl.Execute(ClipperLib::ctDifference, Solution);
    ...
    paths ContourParallel::set_minus(const path &contour, const paths &holes, bool onlyouter /*=false*/, bool wash /*=true*/, double washdis /*=0.5*/, int num_least /*=50*/)
    {
        ...
        ClipperLib::Clipper cl_hole;
        for (int i_hole = 0; i_hole < holes.size(); ++i_hole)
        {
            ClipperLib::Path Hole;
            for (int i = 0; i < holes[i_hole].length; ++i)
            {
                Hole << ClipperLib::IntPoint(double2cInt(holes[i_hole].x[i], scale, delta_x), double2cInt(holes[i_hole].y[i], scale, delta_y));
            }
            Hole << Hole[0];
            if (i_hole == 0)
            {
                cl_hole.AddPath(Hole, ClipperLib::ptSubject, true);
            }
            else
            {
                cl_hole.AddPath(Hole, ClipperLib::ptClip, true);
            }
        }
        cl_hole.Execute(ClipperLib::ctUnion, Holes);

        ClipperLib::Paths IntersectHoles;
        for (int i_hole = 0; i_hole < Holes.size(); ++i_hole)
        {
        ...
        ClipperLib::Clipper cl;
        cl.AddPath(Contour, ClipperLib::ptSubject, true);
        cl.AddPaths(IntersectHoles, ClipperLib::ptClip, true);
        cl.Execute(ClipperLib::ctDifference, Solution);
        for (int i_path = 0; i_path < Solution.size(); ++i_path)
        {
            if (Solution[i_path].size() < 3)
    ...
    paths ContourParallel::tool_compensate(const path &contour, const paths &holes, double dis, bool wash /*=true*/, double washdis /*=0.5*/, int num_least /*=50*/)
    {
        ...
        ClipperLib::Paths ContourOffset;
        ClipperLib::ClipperOffset co;
        co.AddPath(Contour, ClipperLib::jtRound, ClipperLib::etClosedPolygon);
        co.Execute(ContourOffset, dis * scale);
        co.Clear();

        ClipperLib::Paths HolesOffset;
        for (int i_hole = 0; i_hole < holes.size(); ++i_hole)
        {
            ClipperLib::Path Hole;
            ClipperLib::Paths HoleOffset;
            for (int i = 0; i < holes[i_hole].length; ++i)
            {
                Hole << ClipperLib::IntPoint(double2cInt(holes[i_hole].x[i], scale, delta_x), double2cInt(holes[i_hole].y[i], scale, delta_y));
            }
            Hole << Hole[0];
            co.AddPath(Hole, ClipperLib::jtRound, ClipperLib::etClosedPolygon);
        ...
        {
            ClipperLib::Clipper cl;
            cl.AddPaths(ContourOffset, ClipperLib::ptSubject, true);
            cl.AddPaths(HolesOffset, ClipperLib::ptClip, true);
            cl.Execute(ClipperLib::ctDifference, Solution);
        }
        else
        {
    ...
    bool ContourParallel::path_subset(const ClipperLib::Path &p1, const ClipperLib::Path &p2)
    {
        ClipperLib::Paths Solution;
        ClipperLib::Clipper cl;
        cl.AddPath(p1, ClipperLib::ptSubject, true);
        cl.AddPath(p2, ClipperLib::ptClip, true);
        cl.Execute(ClipperLib::ctDifference, Solution);
        return !Solution.size();
    }

    // Transform from a path variable to a Path variable
    ClipperLib::Path ContourParallel::path2Path(const path &p, double scale, double delta_x /*=0.0*/, double delta_y /*=0.0*/)
    {
        ClipperLib::Path P;
        for (int i = 0; i < p.length; ++i)
        {
            P << ClipperLib::IntPoint(double2cInt(p.x[i], scale, delta_x), double2cInt(p.y[i], scale, delta_y));
        }
        P << P[0];
        return P;
    }
