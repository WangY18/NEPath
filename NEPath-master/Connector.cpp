...
path Connector::ConnectedFermatSpiral_MultMinimum(pathnode* root, double dis, double in/*=-1.0*/) {
	...
	pathnode* root2 = root;
	while (true) { // Find the first fork or leaf
	...
	while (!S_top.empty()) {
		...
		if (child->children.size() > 1) { // Branches.
		...
		else { // Leaf. Directly apply FermatSpiral_SingleMinimum.
			pathnode* write = NULL;
			...
			if (!parent->parent) {
				if (root == root2) { // Prevent repeated calculations in the final step
					continue;
				}
				double in_now = -1.0;
				if (!x_in.empty()) {
...
path Connector::FermatSpiral_SingleMinimum(pathnode* root, double dis, double in/*=-1.0*/) {
	if (!root->children.size()) { // Only a level
	...
	while (root) {
		...
		else { // This circle is large enough.
			if (std::abs(in - out) < 1e-6) {
				if (!first_circle) { // Consider the last upper circle.
					...
					if (min(dis0in + dis1out, dis1in + dis0out) < min(dis0in + dis2out, dis2in + dis0out)) { // The point close the the in-point and out-point in the last circle as out-point in this circle.
						if (dis0in + dis1out <= dis1in + dis0out) { // 0-in, 1-out
						...
						if (dis0in + dis2out <= dis2in + dis0out) { // 0-in, 1-out
				...
				if (min(dis_in0in + dis_out1out, dis_in0out + dis_out1in) <=
					...
					if (dis_in0in + dis_out1out < dis_in0out + dis_out1in) { // in0-in, out1-out
						out = out1; // in' = in; out' = backdis(out) > in
					}
					else { // in0-out, out1-in
						out = in;
						in = out1;
					...
					if (dis_in1in + dis_out0out < dis_in1out + dis_out0in) { // in1-in, out0-out
						in = in1;
					}
					else { // in1-out, out0-in
						in = out;
						out = in1;
						out_forward_in = !out_forward_in;
					}
		...
		if (root->children.size()) {
			...
			if (subset_cycle(in, out, idm, true, false)) { // Counter clockwise, idm between in-point and out-point
				for (int i = ceil(in); i != floor(out); i = (i + 1) % root->data.length) {
					xin.push_back(root->data.x[i]);
					yin.push_back(root->data.y[i]);
				}
			}
			else { // Clockwise, idm between out-point and in-point
