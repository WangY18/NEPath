...
#include "../../NEPath-master/Curve.h"
#include "../../NEPath-master/PlanningOptions.h"
#include "../../NEPath-master/path.h"
...
NB_MODULE(_nepath, m) {
    ...
    nb::class_<path>(m, "Path")
        ...
        .def("get_arrays", [](const path& p) {
            ...
            return nb::make_tuple(x_list, y_list);
        }, "Get x and y coordinates as Python lists")
        .def("set_arrays", [](path& p,
                             nb::ndarray<double, nb::shape<-1>, nb::c_contig> x_array,
                             nb::ndarray<double, nb::shape<-1>, nb::c_contig> y_array) {
            size_t length = x_array.shape(0);
            if (length != y_array.shape(0)) {
                throw std::runtime_error("x and y arrays must have the same length");
        ...
        .def_static("from_arrays", [](
            nb::ndarray<double, nb::shape<-1>, nb::c_contig> x_array,
            nb::ndarray<double, nb::shape<-1>, nb::c_contig> y_array) {
            return numpy_to_path(x_array, y_array);
        }, "x"_a, "y"_a, "Create a Path from numpy arrays");
    ...
    nb::class_<ContourParallelOptions>(m, "ContourParallelOptions")
        ...
        .def_rw("connect", &ContourParallelOptions::connect, "Connection algorithm");
    ...
    nb::class_<NonEquidistantOptions>(m, "NonEquidistantOptions")
        ...
        .def_rw("connect", &NonEquidistantOptions::connect, "Connection algorithm");
    #endif
    ...
    nb::class_<UnderFillSolution>(m, "UnderFillSolution")
        ...
        .def("get_xs", [](const UnderFillSolution& sol) {
            return std::vector<double>(sol.xs, sol.xs + sol.nx);
        }, "Get xs array")
        .def("get_ys", [](const UnderFillSolution& sol) {
            return std::vector<double>(sol.ys, sol.ys + sol.ny);
        }, "Get ys array")
    ...
    nb::class_<SharpTurnSolution>(m, "SharpTurnSolution")
        ...
        .def("get_area_percent", [](const SharpTurnSolution& sol) {
            return std::vector<double>(sol.AreaPercent, sol.AreaPercent + sol.length);
        }, "Get AreaPercent array")
        .def("get_sharp_turn", [](const SharpTurnSolution& sol) {
            return std::vector<bool>(sol.SharpTurn, sol.SharpTurn + sol.length);
        }, "Get SharpTurn array")
    ...
    nb::class_<NEPathPlanner>(m, "NEPathPlanner")
        ...
        .def("set_contour",
             [](NEPathPlanner& self,
                nb::ndarray<double, nb::shape<-1>, nb::c_contig> x_array,
                nb::ndarray<double, nb::shape<-1>, nb::c_contig> y_array,
                bool wash = true, double washdis = 0.2, int num_least = 50) {
                 path p = numpy_to_path(x_array, y_array);
                 self.set_contour(p, wash, washdis, num_least);
             },
             "x"_a, "y"_a, "wash"_a = true, "washdis"_a = 0.2, "num_least"_a = 50,
             "Set the contour from numpy arrays")
        .def("addhole",
             nb::overload_cast<const path&, bool, double, int>(&NEPathPlanner::addhole),
             "hole"_a, "wash"_a = true, "washdis"_a = 0.2, "num_least"_a = 50,
        ...
        .def("addhole",
             [](NEPathPlanner& self,
                ...
                bool wash = true, double washdis = 0.2, int num_least = 50) {
                 path p = numpy_to_path(x_array, y_array);
                 self.addhole(p, wash, washdis, num_least);
             },
             "x"_a, "y"_a, "wash"_a = true, "washdis"_a = 0.2, "num_least"_a = 50,
             "Add a hole from numpy arrays")
        .def("addholes", &NEPathPlanner::addholes,
             "holes"_a, "wash"_a = true, "washdis"_a = 0.2, "num_least"_a = 50,
             "Add multiple holes (inner boundaries) onto the slice")
        .def("tool_compensate", &NEPathPlanner::tool_compensate,
             "opts"_a,
             "Offset the contour and holes of the slice with a distance")
        #ifdef ENABLE_DIRECTION_PARALLEL
        .def("Raster", &NEPathPlanner::Raster,
             "opts"_a,
             "Generate Raster toolpath")
        .def("Zigzag", &NEPathPlanner::Zigzag,
             "opts"_a,
    ...
    nb::class_<Curve>(m, "Curve")
        ...
        .def_static("wash_dis",
                   nb::overload_cast<const path&, double>(&Curve::wash_dis),
                   "p"_a, "dis"_a,
