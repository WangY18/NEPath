...
double Curve::BackDis(const double* x, const double* y, int length, double id, double dis) {
	...
	for (register int i = (id_floor + length - 1) % length; ; ) {
		...
		if (sumL + dl <= dis) {
			...
			double alpha = (dis - sumL) / dl; // alpha越小，越靠近i+1
...
UnderFillSolution Curve::UnderFill(const path& contour, const paths& holes, const paths& ps, double delta, double reratio) {
	...
	for (int i = 0; i <= holes.size(); ++i) {
		for (int j = 0; j < all_contours[i]->length; ++j) { // 沿x切割，各线段包含起点、不包含终点
			if (all_contours[i]->x[j] == all_contours[i]->x[(j + 1) % all_contours[i]->length]) {
				...
				for (int k = ceil((all_contours[i]->x[j] - sol.xs[0]) / reratio) - 1; k < sol.nx && sol.xs[k] < all_contours[i]->x[(j + 1) % all_contours[i]->length]; ++k) {
					if (k < 0 || sol.xs[k] < all_contours[i]->x[j]) { // 防止数值误差（齿轮案例）
						continue;
					}
...
double Curve::AreaInvariant_OnePoint(const path& p, double radius, int id, bool close) {
	...
	// 首先找到交点
	if (!close) { // 不闭合
		for (int i = id + 1; i < p.length; ++i) { // 搜索plus
			if (dis(xstar, ystar, p.x[i], p.y[i]) >= radius) {
				double h = abs(cropro(xstar - p.x[i - 1], ystar - p.y[i - 1], xstar - p.x[i], ystar - p.y[i])) / dis(p.x[i], p.y[i], p.x[i - 1], p.y[i - 1]); // 三角形的高
		...
		for (int i = (id + 1) % p.length; subset_cycle((id + 1) % p.length, id, i, true, false); i = (i + 1) % p.length) { // 搜索plus
			if (dis(xstar, ystar, p.x[i], p.y[i]) >= radius) {
				int j = (i + p.length - 1) % p.length;
				double h = abs(cropro(xstar - p.x[j], ystar - p.y[j], xstar - p.x[i], ystar - p.y[i])) / dis(p.x[i], p.y[i], p.x[j], p.y[j]); // 三角形的高
				double lstar = sqrt(radius * radius - h * h);
		...
		for (int i = (id + p.length - 1) % p.length; subset_cycle(id, (id + p.length - 1) % p.length, i, false, true); i = (i + p.length - 1) % p.length) { // 搜索minus
			if (dis(xstar, ystar, p.x[i], p.y[i]) >= radius) {
				int j = (i + 1) % p.length;
				double h = abs(cropro(xstar - p.x[j], ystar - p.y[j], xstar - p.x[i], ystar - p.y[i])) / dis(p.x[i], p.y[i], p.x[j], p.y[j]); // 三角形的高
				double lstar = sqrt(radius * radius - h * h);
	...
	// idplus, idminus不会过近，其中间至少间隔一个id
	// 求面积A1
	double Area1 = 0.0;
