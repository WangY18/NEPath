...
namespace nepath
{
        ...
        for (int i = 0; i <= holes.size(); ++i)
        {
            ...
            { // ��x�и���߶ΰ�����㡢�������յ�
                ...
                else if (all_contours[i]->x[j] < all_contours[i]->x[(j + 1) % all_contours[i]->length])
                {
                    ...
                    {
                        if (k < 0 || sol.xs[k] < all_contours[i]->x[j])
                        { // ��ֹ��ֵ�����ְ�����
                            continue;
                        }
    ...
    double Curve::AreaInvariant_OnePoint(const path &p, double radius, int id, bool close)
    {
        ...
        // Firstly, find the intersection points
        if (!close)
        { // Closed curve
            ...
            { // Search for plus
                ...
                {
                    double h = abs(cropro(xstar - p.x[i - 1], ystar - p.y[i - 1], xstar - p.x[i], ystar - p.y[i])) / dis(p.x[i], p.y[i], p.x[i - 1], p.y[i - 1]); // Distance from point to line
        ...
        { // Closed curve
            for (int i = (id + 1) % p.length; subset_cycle((id + 1) % p.length, id, i, true, false); i = (i + 1) % p.length)
            { // Search for plus
                if (dis(xstar, ystar, p.x[i], p.y[i]) >= radius)
                {
                    int j = (i + p.length - 1) % p.length;
                    double h = abs(cropro(xstar - p.x[j], ystar - p.y[j], xstar - p.x[i], ystar - p.y[i])) / dis(p.x[i], p.y[i], p.x[j], p.y[j]); // Distance from point to line
                    double lstar = sqrt(radius * radius - h * h);
            ...
            for (int i = (id + p.length - 1) % p.length; subset_cycle(id, (id + p.length - 1) % p.length, i, false, true); i = (i + p.length - 1) % p.length)
            { // Search for minus
                if (dis(xstar, ystar, p.x[i], p.y[i]) >= radius)
                {
                    int j = (i + 1) % p.length;
        ...
        // idplus, idminus are not too close, with at least one id in between
        // Solve the area A1
        double Area1 = 0.0;
