...
int main()
{
    ...
    cout << "IQOP-Gurobi: There are " << IQOP_paths.size() << " continuous toolpaths in total." << endl;

    FileAgent::mkdir((fs::path(__FILE__).parent_path().parent_path() / "data_examples" / "demo_IQOP_Gurobi" / "paths_IQ").string().c_str(), true);
    FileAgent::write_csv(IQOP_paths, (fs::path(__FILE__).parent_path().parent_path() / "data_examples" / "demo_IQOP_Gurobi" / "paths_IQ" / "").string().c_str(), ".csv");
    // FileAgent::write_csv(contour, (fs::path(__FILE__).parent_path().parent_path() / "data_examples" / "demo_IQOP_Gurobi" / "contour.csv").string().c_str());
    return 0;
}